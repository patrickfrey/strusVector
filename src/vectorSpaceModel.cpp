...
public:
		...
		,m_lshmodel(),m_genmodelmain(),m_genmodelmap(),m_samplear()
	{
		...
		if (m_database->isempty() || m_state < 1)
		{
			...
			for (; mi != me; ++mi)
			{
				m_genmodelmap[ mi->first] = GenModel( m_config.threads, m_config.maxdist, mi->second.simdist, mi->second.raddist, mi->second.eqdist, mi->second.mutations, mi->second.votes, mi->second.descendants, mi->second.maxage, mi->second.iterations, mi->second.assignments, mi->second.greediness, mi->second.isaf, mi->second.eqdiff, mi->second.with_singletons, m_errorhnd);
			}
