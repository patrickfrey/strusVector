	...
	:m_base(base_),m_basesize(basesize_),m_ofs(0),m_size(0),m_shsize(0),m_seed(seed_),m_maxdiff(0)
{
	...
	if (m_basesize <= 0)
	{
		if (m_basesize < 0) throw std::runtime_error( _TXT("illegal range passed to LshBench"));
		return;
	}
	m_shsize = m_base[0].size();
	if (m_shsize == 0) throw std::runtime_error( _TXT("empty simhash values passed to LshBench"));
...
strus::Index LshBench::init( const strus::Index& ofs_)
{
	...
	if (ofs_ >= m_basesize) throw std::runtime_error( _TXT("illegal range passed to LshBench::init"));
	m_ofs = ofs_;
	...
	for (; ai != ae; ++ai)
	{
		if (m_base[m_ofs+ai].size() != m_shsize) throw std::runtime_error( _TXT("empty simhash values passed to LshBench::init"));
