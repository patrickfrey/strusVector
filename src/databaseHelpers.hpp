...
public:
	...
	{
		if (8 + m_itr > MaxKeySize) throw std::runtime_error( _TXT("array bound write in database key buffer"));
...
public:
	...
	DatabaseKeyScanner& operator[]( Index& val)
	{
		...
		if (!keylen || keylen > m_end - m_itr) throw std::runtime_error( _TXT("array bound read in database key scanner"));
...
public:
	...
	DatabaseValueBuffer& operator[]( const ScalarType& val)
	{
		...
		if (sizeof(NetScalarType) + m_itr > MaxValueSize) throw std::runtime_error( _TXT("array bound write in database value buffer"));
...
public:
	...
	DatabaseValueScanner& operator[]( ScalarType& val)
	{
		...
		if (sizeof(NetScalarType) > (unsigned int)(m_end - m_itr)) throw std::runtime_error( _TXT("array bound read in database value scanner"));
		NetScalarType val_n;
