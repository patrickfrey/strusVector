	...
	,with_singletons(defaultcfg.with_singletons)
{
	...
	{}
	if (si != se) throw std::runtime_error( _TXT("unknown configuration parameter for gen model (vector storage)"));
}
...
void GenModelConfig::parse( std::string& src, unsigned int maxdist, ErrorBufferInterface* errorhnd)
{
	...
	if (simdist > maxdist)
	{
		throw std::runtime_error( _TXT("the 'simdist' configuration parameter must not be bigger than 'maxdist'"));
	}
	(void)extractUIntFromConfigString( raddist, src, "raddist", errorhnd);
	if (raddist > simdist)
	{
		throw std::runtime_error( _TXT("the 'raddist' configuration parameter must not be bigger than 'simdist'"));
	}
	(void)extractUIntFromConfigString( eqdist, src, "eqdist", errorhnd);
	if (eqdist > simdist)
	{
		throw std::runtime_error( _TXT("the 'eqdist' configuration parameter must not be bigger than 'simdist'"));
	}
	...
	{
		throw std::runtime_error( _TXT("error in vector storage configuration: mutations, descendants, maxage or iterations values must not be zero"));
	}
	...
	,altgenmap(defaultcfg.altgenmap)
{
	...
	if (!maxconcepts && maxfeatures)
	{
		...
		if (maxfeatures > maxconcepts) throw std::runtime_error( _TXT("'maxconcepts' has to be specified, because default maxconcepts = (maxfeatures * assignments / 2) exceeds domain"));
	}
	...
	while (extractStringFromConfigString( altgencfgstr, src, "altgen", errorhnd))
	{
		...
		{
			if ((*ee|32) < 'a' || (*ee|32) > 'z') throw std::runtime_error( _TXT("expected identifier at start of alternative gen configuration in vector storage"));
			++ee;
		}
		std::string name( namestart, ee - namestart);
		while (*ee && (unsigned char)*ee <= 32) ++ee;
		if (*ee != ':') throw std::runtime_error( _TXT("expected colon ':' after identifier at start of alternative gen configuration in vector storage"));
		++ee;
		GenModelConfig altgencfg( ee, maxdist, errorhnd, gencfg);
		if (altgenmap.find( name) != altgenmap.end()) throw std::runtime_error( _TXT("duplicate definition of alternative gen configuration in vector storage"));
	...
	while (extractStringFromConfigString( condepdefstr, src, "condep", errorhnd))
	{
		...
		for (; *cc && *cc != ':'; ++cc){}
		if (!*cc) throw std::runtime_error( _TXT("illegal concept class dependency definition in config (missing colon ':')"));
	...
	{
		throw std::runtime_error( _TXT("error in vector storage configuration: dim, bits or var values must not be zero"));
	}
