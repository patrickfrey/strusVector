...
std::vector<Vector> readVectorsFromFile( strus::DataRecordFile& vecfile)
{
	...
	rt.reserve( vecfile.size());
	std::size_t vecsize = vecfile.recordsize() / sizeof(double);
	double const* ri = (const double*)vecfile.read( 0);
	std::size_t ridx = 0;
	for (; ri; ri = (const double*)vecfile.read( ++ridx))
	{
		Vector elem;
		rt.reserve( vecfile.recordsize() / sizeof(double));
		const double* re = ri + vecsize;
		for (; ri != re; ++ri)
		{
			elem.push_back( ByteOrder<double>::ntoh( *ri));
		}
		rt.push_back( elem);
	}
	return rt;
...
Vector readVectorFromFile( std::size_t index, strus::DataRecordFile& vecfile)
{
	Vector rt;
	rt.reserve( vecfile.recordsize() / sizeof(double));
	double const* ri = (const double*)vecfile.read( index);
	const double* re = ri + vecfile.recordsize() / sizeof(double);
	for (; ri != re; ++ri)
	{
...
{
	std::vector<Vector>::const_iterator vi = vectors.begin(), ve = vectors.end();
	for (; vi != ve; ++vi)
	{
		if (vecfile.recordsize() != vi->size() * sizeof(double))
		{
			throw strus::runtime_error(_TXT("failed to append vectors to file because of size mismatch"));
		}
		...
		for (; xi != xe; ++xi)
		{
			val.push_back( ByteOrder<double>::hton( *xi));
		}
		vecfile.append( val.data());
	}
}
