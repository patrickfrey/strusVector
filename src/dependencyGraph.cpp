...
DependencyGraph strus::buildGroupDependencyGraph( std::size_t nofSamples, std::size_t nofGroups, const SampleSimGroupMap& sampleSimGroupMap, const SimGroupMap& groupMap, float isaf)
{
	...
	for (; si < se; ++si)
	{
		...
		for (; ni != ne; ++ni)
		{
			...
			if (!group_n.get()) throw strus::runtime_error(_TXT("internal: inconsistency in group map"));
			...
			for (; oi != oe; ++oi)
			{
				if (*oi != *ni)
				{
					...
					if (!group_o.get()) throw strus::runtime_error(_TXT("internal: inconsistency in group map"));
