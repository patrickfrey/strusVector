...
bool SimGroupMap::Node::remove( const FeatureIndex& gidx)
{
	FeatureIndex ii=0;
...
bool SimGroupMap::Node::insert( const FeatureIndex& gidx)
{
...
bool SimGroupMap::Node::contains( const FeatureIndex& gidx) const
{
	FeatureIndex ii=0;
	for (; ii<size && groupidx[ii] < gidx; ++ii){}
	if (ii==size) return false;
